...
function fqBrand(id,name){
	...
	if(null != price){
		//后台请求 js 的get提交
		window.location.href = "/product/display/list.shtml?keyword=" + keyword + "&brandId=" + brandId + "&brandName=" + brandName + "&price=" + price;
	}else{
		//后台请求 js 的get提交
		window.location.href = "/product/display/list.shtml?keyword=" + keyword + "&brandId=" + brandId + "&brandName=" + brandName;
	}
}
//点击价格
function fqPrice(pp){
...
function login(){
	...
	window.location.href = "/shopping/login.shtml?returnUrl=" +  encodeURIComponent(window.location.href); //GET POST
}
function myOrder(){
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<c:forEach items="${keys }" var="k">
					<a href="javascript:void(0);" onclick="clearQuery('${k.key }')">
					<em>${k.key }：</em>${k.value }
				...
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brands }" var="brand">
						<a href="javascript:void(0);" onclick="fqBrand('${brand.id}','${brand.name }')" title="${brand.name }">${brand.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
			...
			<div class="mt ofc">
				...
				<c:forEach items="${pagination.list }" var="product">
					<li>
						<a href="javascript:void(0)"  onclick="window.open('/product/detail.shtml?id=${product.id}')"  title="${product.name}" class="pic"><img src="${product.img.allUrl }" alt="${product.name}" /></a>
						<dl>
							<!-- dt 10个文字+... -->
