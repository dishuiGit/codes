...
//修改
function modify(name,addr,phone,isDef){
	//收货人
	$("#username").val(name);
	//默认收货地址
	if(isDef == 1){
			...
			<div class="ofc">
				...
				<tbody>
					...
					<tr <c:if test="${addr.isDef == 1 }">class="here"</c:if>> 
						...
						<td>${addr.addr }</td>
						<td>${addr.phone }</td>
						<td><a href="javascript:void(0);" title="修改" onclick="modify('${addr.name }','${addr.addr }','${addr.phone }','${addr.isDef}')" class="blue">[修改]</a><a href="javascript:void(0);" title="删除" onclick="del(this)" class="blue">[删除]</a></td>
					</tr>
