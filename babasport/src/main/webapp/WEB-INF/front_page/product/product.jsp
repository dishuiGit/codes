...
//单品页
function detail(id){
	window.open("/product/display/productDetail.shtml?id="+id);
}
//登陆
function login(){
...
//搜索
var baseurl = "/product/display/list.shtml?";
//keyword
...
//叉叉
function filterId(obj,value){
	//alert(1);
	$(obj).hide();
	//if($(obj).attr(""))
	if(value == '品牌'){
		//alert(1);
		window.location.href = baseurl+"keyword="+$("#keyword").val()+"&price="+price;
	}
	if(value == '价格'){
		//alert(1);
		window.location.href = baseurl+"keyword="+$("#keyword").val()+"&brandId="+brandId;
	}
}
//品牌查询
function brandQ(obj){
	//alert($(obj).attr("brandId"));
	brandId = $(obj).attr("brandId");
	//alert($("#keyword").val());
	//$("#filterId").html("<em>品牌：</em>"+$(obj).attr("title"));
	if(keyword==''){
		window.location.href = baseurl+"brandId="+brandId;
	}else{
		window.location.href = baseurl+"keyword="+$("#keyword").val()+"&brandId="+brandId;
	}
}
...
function priceQ(obj){
	...
	price = $(obj).html();
	if(keyword == '' && brandId == ''){
		window.location.href = baseurl+"price="+price;
	}else {
		window.location.href = baseurl+"keyword="+$("#keyword").val()+"&brandId="+brandId+"&price="+price;
	}
}
</script>
</head>
...
<div class="w loc">
	<div class="h-title">
	    ...
	    <div class="h-search">
	      	<input id="keyword" type="text" value="${param.keyword }"/>
	        <div class="h-se-btn"><a href="javascript:void(0);" id="serach">搜索</a></div>
	    </div>
	</div>
	<dl id="cart" class="cart r">
		<dt><a href="/shopping/buyCart.shtml" title="结算">结算</a>购物车:<b id="">5</b>件</dt>
		<dd class="hidden">
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			...
			<c:if test="${flag}">
				...
				<p class="sel">
					...
					<a onclick="filterId(this,'${filter.key}')" href="javascript:void(0);">
					<em>${filter.key}：</em>${filter.value}
			...
			<c:if test="${brandId == null }">
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brandList}" var="brand">
						<a href="javascript:void(0);" brandId='${brand.id}' title="${brand.name }" onclick="brandQ(this)">${brand.name }</a>
					</c:forEach>
				</p></li>
			</c:if>
			<c:if test="${price == null }">
				<li><b>价格：</b><p>
					<a href="javascript:void(0);" title="不限" class="here">不限</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="1-99">0-79</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="100-199">80-199</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="200-499">200-299</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="200-499">300-499</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="200-499">500-599</a>
					<a href="javascript:void(0);" onclick="priceQ(this)" title="200-499">600以上</a>
				</p></li>
			</c:if>
				<li><b>类型：</b><p>
					<a href="javascript:void(0);" title="不限" class="here">不限</a>
			...
			<div class="mt ofc">
				...
				<c:forEach items="${pagination.list }" var="product">
					<li>
						<a href="/html/${product.id}.html" title="瑜伽服" class="pic"><img src="${product.img.allUrl}" alt="瑜伽服" /></a>
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="javascript:void(0)" onclick="detail('${product.id}')" title="${product.name}" >${product.name}</a></dt>
							<!-- dt 25个文字+... -->
