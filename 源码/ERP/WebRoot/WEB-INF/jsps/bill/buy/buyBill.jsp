	...
	*/
	...
	$(function(){
		...
		$(".info").click(function(){
			...
			//发送ajax请求
			$.post("bill_ajaxGetInfo.action",jsonParam,function(data){
				$(".ajaxMsg").remove();
				
				//data是个数组
				//数组中包含的json对象具有多个属性
				//num,priceView,totalPriceView,om
				//om中包含orderNum,createTimeView
				...
				for(var i = 0;i<odmList.length;i++){
					var odm = odmList[i];	//num,priceView,totalPriceView
					var om = odm.om;		//orderNum,createTimeView
...
<div class="content-right">
	...
	<div class="content-text">
		<s:form action="bill_buyBill.action" method="post">
			<div class="square-o-top">
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td width="70">订单类别:</td>
						<td width="190">
							<s:select name="bqm.type" list="@cn.itcast.erp.invoice.order.vo.OrderModel@buyTypeMap" headerKey="-1" headerValue="---请-选-择---" cssStyle="width:113px"></s:select>
						</td>
						<td width="70">开始日期:</td>
						<td width="190">
					...
					<tr>
						...
						<td>厂商名称:</td>
						<td>
							<s:select name="bqm.supplierUuid" list="supplierList" listKey="uuid" listValue="name" headerKey="-1" headerValue="---请-选-择---" cssStyle="width:113px"></s:select>
						</td>
						<td>结束日期:</td>
						<td width="190">
							<input type="text" size="18" onfocus="c.showMoreDay=false;c.show(this);" />
						<td>
							<a href="bill_downloadExcelBill.action?bqm.type=${param['bqm.type']}&bqm.supplierUuid=${bqm.supplierUuid}">
			...
			<div class="square-order">
				...
				<div style="float:right;">
					<a href="bill_getPieBill.action?bqm.type=${param['bqm.type']}&bqm.supplierUuid=${bqm.supplierUuid}">
						<img id="pei" src="bill_getPieBill.action?bqm.type=${param['bqm.type']}&bqm.supplierUuid=${bqm.supplierUuid}" width="228px" height="180px">
					</a>
				</div>
			</div>
		</s:form>
