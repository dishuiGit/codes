...
//sku联动
//尺码变色
function sizeToRed(obj){
	if($(obj).attr("class")=="not-allow"){
		return;
	}
	//清空
	...
	<c:forEach items="${skus}" var="sku">
		...
		//颜色/尺码一样
		//alert($(obj).attr("id"));
		if(colorId == '${sku.colorId}' && $(obj).attr("id") == '${sku.size}'){
			...
			//确定skuId
			skuId = '${sku.id}';
			//console.log('${sku.skuPrice}');
			//console.log('${sku.marketPrice}');
			//console.log('${sku.deliveFee}');
		}
		//alert('${sku.size}'.indexOf($(obj).attr("id"))));
		/*if('${sku.size}'==$(obj).attr("id")){
			$("#skuPrice").html(￥+'${sku.skuPrice}');
			$("#marketPrice").html(￥+'${sku.marketPrice}');
			$("#deliveFee").html('${sku.deliveFee}'+元);
		}*/
...
function colorToRed(obj,id){
	...
	if('${sku.colorId}'==id){
		...
		//默认第一个边框变红
	$("#sizes a:[class=changToWhite]").first().attr("class","changToRed");
	$("#sizes a:[class=changToRed]").first().click();
}
...
function addCart(){
	...
	var amount = $("#num").val();
	//alert(amount);
	window.location.href='/shopping/buyCart.shtml?skuId='+skuId+"&amount="+amount;
}
//立即购买
function buy(){
	window.location.href='/shopping/buyCart.shtml?skuId='+skuId;
}
</script>
</head>
<body>
...
<div class="w ofc mt">
	<div class="l">
		<div class="showPro">
			<div class="big"><a id="showImg" class="cloud-zoom" href="${imgAllUrl }" rel="adjustX:10,adjustY:-1"><img alt="" src="${imgAllUrl }"></a></div>
		</div>
	</div>
	<div class="r" style="width: 640px">
		<ul class="uls form">
			<li><h2>${product.name }</h2></li>
			<li><label>巴  巴 价：</label><span class="word"><b id="skuPrice" class="f14 red mr">￥${skus[0].skuPrice }</b>(市场价:<del id="marketPrice">￥${skus[0].marketPrice }</del>)</span></li>
			<li><label>商品评价：</label><span class="word"><span class="val_no val3d4" title="4分">4分</span><var class="blue">(已有888人评价)</var></span></li>
			<li><label>运　　费：</label><span class="word" id="deliveFee">${skus[0].deliveFee}元</span></li>
			...
			<li id="sizes"><label>尺　　码：</label>
						<a href="javascript:void(0)" onclick="sizeToRed(this)" class="not-allow"  id="S">S</a>
						<a href="javascript:void(0)" onclick="sizeToRed(this)" class="not-allow"  id="M">M</a>
						<a href="javascript:void(0)" onclick="sizeToRed(this)" class="not-allow"  id="L">L</a>
						<a href="javascript:void(0)" onclick="sizeToRed(this)" class="not-allow"  id="XL">XL</a>
						<a href="javascript:void(0)" onclick="sizeToRed(this)" class="not-allow"  id="XXL">XXL</a>
			</li>
